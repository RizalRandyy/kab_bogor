...
class Rab extends My_Controller
{
	...
	public function index_get()
	{
		...
		$this->data['title'] = ucfirst($segment) . 'RENCANA ANGGARAN BIAYA (RAB)';
		$this->data['page'] = 'rab';
		$this->data['version'] = $this->uri->segment(1);
		$this->data['id'] = @$this->get('id') ?: null;

		$this->data['js'] = array(
			'assets/js/app/rab.js?' . rand(),
	...
	{
		$return = $this->Rab_model->getkegiatan();
	...
	{
		$id = $this->get('id', TRUE);
		$data = $this->Rab_model->getDetailByKegiatan($id);
	...
	public function saveData_get()
	{
		...
		foreach ($id_thn_harga as $key => $idHarga) {
			...
			$item = $this->Rab_model->getHargaById($idHarga);
		...
		$this->pdf->load_view('report/rab', $result);
	}
}
