...
class Tahun_kegiatan_asb_model extends CI_Model
{
    ...
    ');
        ...
        $this->db->from('tb_standar_biaya_thn');
        $this->db->join('tb_standar_biaya', 'tb_standar_biaya_thn.idASB = tb_standar_biaya.id');
        ...
        $this->db->order_by('tahunASB DESC, tb_standar_biaya_thn.idASB ASC');
    ...
    public function getkegiatan()
    {
        $get_data = $this->db->select('
            ...
            tb_standar_biaya.satuan
        ')
            ->from('tb_standar_biaya')
            ->join('tb_opd', 'tb_standar_biaya.idOpd = tb_opd.idOpd', 'left')
            ->order_by('tb_standar_biaya.idASB')
            ->get()
            ->result_array();
    ...
    {
        $id = decrypt_url($id);
        if ($this->db->delete('tb_standar_biaya_thn', ['id' => $id])) {
            return [
