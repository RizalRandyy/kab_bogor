...
class Dashboard_model extends CI_Model
{
	public function getData($year = '2025')
	{
		...
		if ($qry) {
			...
			foreach ($qry as $key2 => $value2) {
				...
				foreach ($year as $key3 => $value3) {
					...
					if ((int)$value3 == (int)$value2['TahunHarga']) {
						$list[$value2['UraianKelompok'] . " - " . $value2['UraianSpesifikasi'] . " (" . $value2['satuan'] . ")"][$value3] = [
							"year" => $value3,
							"harga" => $value2['harga']
						];
			...
			foreach ($qry as $key4 => $value4) {
				$list[$value4['UraianKelompok'] . " - " . $value4['UraianSpesifikasi'] . " (" . $value4['satuan'] . ")"][$value4['TahunHarga']] = [
					"year" => $value4['TahunHarga'],
	...
	public function getDataLokasi($users)
	{
		$this->db->select('tahun, COUNT(*) as jumlah');
		$this->db->group_by('tahun');
		$this->db->order_by('tahun', 'ASC');
		$get_data = $this->db->get('tb_lokasi')->result_array();
