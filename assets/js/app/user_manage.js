...
console.log('Password:', $scope.password);
                ...
                if ($skip) {
                    ...
                    httpHandler.send({
                        method: 'POST',
                        url: urls + 'user_manage/save_user',
                        data: formData,
                        headers: {
                            'Content-Type': undefined // penting agar browser otomatis set multipart/form-data
                        }
                    }).then(function success(resp) {
                        Swal.close();

                        var result = resp.data; // penting! ambil dari resp.data
                        ...
                        console.error('Error saving user:', errorResp);
    ...
    $scope.delete_user = function (id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                httpHandler.send({
                    method: 'POST',
                    url: urls + "user_manage/delete_user",
                    data: ({
                        'id': id
                    })
    ...
    $scope.update_password = function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: urls + "user_manage/update_password",
                    dataType: "JSON",
                    cache: false,
                    contentType: false,
                    processData: false,
