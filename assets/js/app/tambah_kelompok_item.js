mainApp
	...
	.controller('tambah_kelompok_item', ['$scope', 'httpHandler', '$filter', '$attrs', '$timeout', function ($scope, httpHandler, $filter, $attrs, $timeout) {
		...
		// Watch pilihannya
		$scope.$watch('tipe', function (newVal) {
			if (newVal === 'SSH') {
				$scope.kriteriaList = $scope.kriteriaSSH;
				$scope.kriteria = ''; // reset pilihan
			} else if (newVal === 'SBU') {
				$scope.kriteriaList = $scope.kriteriaSBU;
				$scope.kriteria = '';
			}
		});
		...
		$scope.save = function () {
			...
			} else if (kriteria == null || kriteria == "") {
				return Toast.fire({
					icon: "warning",
