...
.controller('tambah_kegiatan_hspk', ['$scope', 'httpHandler', '$filter', '$attrs', '$timeout', function ($scope, httpHandler, $filter, $attrs, $timeout) {
	...
	if ($scope.id) {
        ...
        $timeout(function() {
    		httpHandler.send({
    			method: 'GET',
    			url: urls + 'kegiatan_hspk/getById',
    			params: {'id': $scope.id}
    		}).then(
    			function successCallbacks(response) {
    				if(response.data.status == 200){
    					...
    					Swal.fire({
    						...
    						if (result.value) {
    							window.location.replace(urls + 'kegiatan_hspk');
    						}
    					});
    				}
	...
	$scope.save = function () {
		...
		httpHandler.send({
			url: urls + 'kegiatan_hspk/saveData',
			data: formData,
			method: 'POST',
			headers: {
