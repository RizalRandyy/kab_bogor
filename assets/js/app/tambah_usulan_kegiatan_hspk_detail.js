mainApp
    ...
    .controller('tambah_usulan_kegiatan_hspk_detail', ['$scope', 'httpHandler', '$filter', '$attrs', '$timeout', function ($scope, httpHandler, $filter, $attrs, $timeout) {
        ...
        $scope.getData = function (pageno = 1) {
            httpHandler.send({
                method: 'GET',
                url: urls + 'usulan_kegiatan_hspk_detail/kegiatan'
            }).then(
        ...
        if ($scope.id) {
            ...
            $timeout(function () {
                httpHandler.send({
                    method: 'GET',
                    url: urls + 'usulan_kegiatan_hspk_detail/getById',
                    params: { 'id': $scope.id }
                }).then(
                    function successCallbacks(response) {
                        ...
                        if (response.data.status == 200) {
                            ...
                            $scope.viewKegiatan = result.idKegiatan + ' - ' + result.UraianKegiatan + ' - (' + result.satuan + ') - ' + result.tahunPekerjaan;
                            ...
                            Swal.fire({
                                ...
                                if (result.value) {
                                    window.location.replace(urls + 'usulan_kegiatan_hspk_detail');
                                }
                            });
                        }
                        $scope.getData();
        ...
        $scope.save = function () {
            ...
            httpHandler.send({
                url: urls + 'usulan_kegiatan_hspk_detail/saveData',
                data: formData,
                method: 'POST',
                headers: {
