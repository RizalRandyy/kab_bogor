mainApp
	...
	.controller('tambah_usulan_kegiatan_asb_detail', ['$scope', 'httpHandler', '$filter', '$attrs', '$timeout', function ($scope, httpHandler, $filter, $attrs, $timeout) {
		...
		$scope.getData = function (pageno = 1) {
			httpHandler.send({
				method: 'GET',
				url: urls + 'usulan_kegiatan_asb_detail/kegiatan'
			}).then(
		...
		if ($scope.id) {
			...
			$timeout(function () {
				httpHandler.send({
					method: 'GET',
					url: urls + 'usulan_kegiatan_asb_detail/getById',
					params: { 'id': $scope.id }
				}).then(
					function successCallbacks(response) {
						...
						if (response.data.status == 200) {
							...
							Swal.fire({
								...
								if (result.value) {
									window.location.replace(urls + 'usulan_kegiatan_asb_detail');
								}
							});
						}
						$scope.getData();
		...
		$scope.save = function () {
			...
			// Kirim ke backend
			httpHandler.send({
				url: urls + 'usulan_kegiatan_asb_detail/saveData',
				data: formData,
				method: 'POST',
				headers: {
